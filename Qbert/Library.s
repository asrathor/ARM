...
Illuminate_RGB_LED			;r0 is input
	...
	LDR r6, =0xE0028004			;set turns them off (using IO0SET)
	STR r5, [r6]			;turn them all off by storing r5 into IOSET
	LDR r6, =0xE002800C			;clear turns them on (using IO0CLR)
...
illuminateLEDs
	...
	LDR r1, =0x000F0000		;r1 has bits 16-19, 1
 	LDR r6, =0xE0028014 		;address for port 1 set register
 	STR r1, [r6]  			;for setting the number of bits
	LDR r7, =0xE002801C  		;address for port 1 clear register
...
read_from_push_btns
	STMFD sp!,{lr,r6,r9}
	LDR r6, =0xE0028010			;set r6 as address of gpio pin port 1 value register
...
display_digit_on_7_seg
    ...
    LDR r6, =0xE002800C  		;base address for gpio pin clear register
